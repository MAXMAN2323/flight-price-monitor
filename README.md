...
监测器也会监控数据源自身是否失效。完整一轮应查询 `56` 个组合；如果一轮中候选数为 `0`，且错误数达到查询数的 `90%` 以上，记为一次完整数据源失败。

默认规则：

- 连续 `3` 次完整数据源失败后，通过 PushPlus 发送故障提醒。
- 如果故障持续，之后每连续失败 `6` 次再提醒一次，避免刷屏。
- 一旦重新拿到候选价格，连续失败计数会清零。
